...
pub fn init(page_root: *sabaton.paging.Root) callconv(.Inline) void {
    ...
    if (!root.memmap.containsAddr(sabaton.fb.addr)) {
        ...
        sabaton.paging.map(sabaton.fb.addr, sabaton.fb.addr, fbsz, .rw, .writethrough, page_root);
        sabaton.paging.map(sabaton.upper_half_phys_base + sabaton.fb.addr, sabaton.fb.addr, fbsz, .rw, .writethrough, page_root);
    }
